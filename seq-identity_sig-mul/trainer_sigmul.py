...
def main():
    ...
    ckpt_name = f"{model_name}_{data_name}-{monitor}_lr-{args.lr}_loss-{args.loss}"
    model_checkpoint = ModelCheckpoint(
        (
            f"checkpoints_sigmul_rmabnormal/{data_name}"
            if args.rm_abnormal
            else f"checkpoints_sigmul/{data_name}"
        ), 
        monitor=monitor,
        mode=mode,
        filename=ckpt_name,
    ...
    if not args.rm_abnormal:
        metrics_folder = f"results_sigmul/{model_name}/{data_name}_{args.loss}"
    else:
        metrics_folder = f"results_rmabnormal_sigmul/{model_name}/{data_name}_{args.loss}"
